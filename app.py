...
def generate_input(input_type, image=None, text=None, response_amount=3):
    ...
    # handle both text and image input if chosen
        ...
        image_caption = processor.decode(out[0],
                                         skip_special_tokens=True)  # repeat image processing + caption generation and decoding
...
with gr.Blocks() as demo:
    ...
    text_input = gr.Textbox(label="Enter Text Query", placeholder="Enter a description or query here",
    ...
    submit_button.click(fn=generate_input, inputs=[input_type, image_input, text_input, selected_response_amount],
                        outputs=output)
demo.launch()
